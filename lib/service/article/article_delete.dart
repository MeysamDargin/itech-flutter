...
class ArticleDeleteService {
  final String _baseUrl = '${ApiAddress.baseUrl}${ApiEndpoint.articleDelete}';
  final LoginService _loginService = LoginService();

  Future<Map<String, dynamic>> deleteArticle(String articleId) async {
    try {
      print('Attempting to delete article with ID: $articleId');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        return {
          'status': 'error',
          'message': 'Error deleting article: ${response.statusCode}',
        };
      }
    } catch (e) {
      print('Exception while deleting article: $e');
      return {'status': 'error', 'message': 'Error deleting article: $e'};
    }
  }
}
