...
import 'package:itech/service/saved/create_category_service.dart';
import 'package:itech/utils/save/fach_save_directory.dart';

class CreateSaveDirectorySheet extends StatefulWidget {
  final Function? onCollectionCreated;

  const CreateSaveDirectorySheet({Key? key, this.onCollectionCreated})
    : super(key: key);

  @override
  State<CreateSaveDirectorySheet> createState() =>
      _CreateSaveDirectorySheetState();
}

class _CreateSaveDirectorySheetState extends State<CreateSaveDirectorySheet> {
  final TextEditingController _name = TextEditingController();
  final CreateCategoryService _createCategoryService = CreateCategoryService();
  ...
  Future<void> _sendNewCollection() async {
    ...
    try {
      final success = await _createCategoryService.createSaveDirectory(
        name: _name.text,
      );

      if (success != null && success['status'] != 'error') {
        // Fetch updated category list
        await fetchAndParseCategoryList();

        // Close the bottom sheet
        Navigator.pop(context);

        // Call the callback if provided to refresh the parent page
        if (widget.onCollectionCreated != null) {
